...
- A pushed stable tag must be exactly `vMAJOR.MINOR.PATCH` and point to the current `origin/master`. Its workflow validates the debug build, unit tests, lint, and coverage gate, then publishes `qr-code-simple-<version>.apk`, `qr-code-simple-<version>.aab`, `version.json`, the compatibility alias `app-release.apk`, and any usable delta patches to GitHub Releases with the same certificate as the main-branch Debug APK. Delta sources span both channels: a stable also patches from the 4 most recent archived betas, and a beta also patches from the 2 most recent stables, so stable and beta installs can switch channels incrementally.
