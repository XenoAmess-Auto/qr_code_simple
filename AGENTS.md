...
- CI lives in `.github/workflows/build.yml` (build + unit tests on push/PR to `master`/`main`).
- Coverage is generated by JaCoCo (`./gradlew :app:jacocoTestReport`) and deployed to GitHub Pages by the `coverage-pages` job.
