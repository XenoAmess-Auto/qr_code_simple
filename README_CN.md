...
- ✅ **全文搜索** - 按内容、时间、类型搜索。
- ✅ **高级筛选** - 时间范围（今天 / 近 7 天 / 近 30 天）、类型、条码格式筛选，新旧排序切换。
- ✅ **统计图表** - 近 14 天扫码次数柱状图。
...
- ✅ **连续扫描模式** - 无需确认连续扫描多个码，保留精确格式，可选自动保存，并支持 CSV / JSON / Excel 会话导出。
- ✅ **震动反馈** - 识别成功时震动提示。
...
- ✅ **矢量导出** - SVG 格式（无损放大）。
- ✅ **位图导出选项** - PNG / JPEG / WebP，512 / 1024 / 2048 尺寸可选。
- ✅ **分享模板** - 生成带说明文字的分享图片。
...
- ✅ **隐私模式** - 无痕扫描，不写入历史记录。
- ✅ **应用锁** - 指纹 / 密码保护敏感历史。
- ✅ **本地加密** - SQLCipher (AES-256) 加密历史数据库（加密状态与重置入口在 关于 → 隐私设置）。
- ✅ **崩溃日志** - 本地崩溃记录（不上报），关于页可查看 / 分享 / 清除。

> **隐私说明**：`INTERNET` 权限仅用于可选功能：黑名单静默在线更新（默认关闭）、应用更新检查（Stable 走 GitHub Releases，Beta 走本项目 GitHub Pages 端点且只能在“关于”页手动发起）、WebDAV 备份同步（仅用户手动触发）。GitHub 资产下载支持公共加速镜像轮询；产物完整性由 SHA-256 与签名校验保证，不依赖镜像可信。其余功能完全离线可用。

### 界面与体验

- ✅ **亮暗主题** - 跟随系统 / 亮色 / 暗色三档切换。
- ✅ **响应式导航** - 五个主页面使用底部导航；独立结果页保留返回导航和操作菜单。
- ✅ **横屏与平板** - 横屏布局优化；平板（sw600dp+）历史页列表-详情双栏、生成页表单-预览双栏、About/图片扫描居中限宽。
- ✅ **快捷方式** - 长按图标快速扫码或生成。
- ✅ **快捷设置磁贴** - 下拉栏一键进入相机扫描。
- ✅ **桌面小组件** - 快速扫描、快速生成小组件。
...
- ✅ **大图加载优化** - 大图加载内存优化。
- ✅ **可取消生成** - 预览防抖、生命周期感知的回扫校验，以及串行保存 / 分享，避免旧图覆盖和重复写入。
